...
def main():
    ...
    if sistema == 'Darwin':
        ...
        comandosInstalacion.append("brew install protobuf@3.16")
    ...
    if not os.path.exists(f'{ruta_actual}/tests/app/comunicacion_b-f/{backend_ejecutable}'):
        
        comandoCompilacionBackend = f'cd {ruta_actual} && g++ -std=c++20 -Ibackend/include -Ibackend/SoapComm tests/app/comunicacion_b-f/test_backend.cpp backend/src/protoserver.cpp backend/SoapComm/sopasockets.cpp backend/src/mensajes.pb.cc backend/src/search_algorithm.cpp -L/usr/local/lib -lprotobuf -labsl_base -labsl_strings -labsl_time -labsl_log_internal_check_op -labsl_log_internal_conditions -labsl_log_internal_fnmatch -labsl_log_internal_format -labsl_log_internal_globals -labsl_log_internal_log_sink_set -labsl_log_internal_message -labsl_log_internal_nullguard -labsl_log_internal_proto -pthread -o tests/app/comunicacion_b-f/backend'
    ...
    comandoBackend = f'{ruta_actual}/tests/app/comunicacion_b-f/{backend_ejecutable}'
    comandoFronted = f'python {ruta_actual}/tests/app/comunicacion_b-f/test_frontend.py'
